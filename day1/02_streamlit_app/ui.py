...
def display_history_list(history_df):
    ...
    for i, row in paginated_df.iterrows():
        timestamp_str_raw = row["timestamp"]
        try:
            timestamp_utc = datetime.fromisoformat(timestamp_str_raw)
        except ValueError:
            timestamp_utc = datetime.strptime(timestamp_str_raw, "%Y-%m-%d %H:%M:%S")
        timestamp_utc = timestamp_utc.replace(tzinfo=pytz.UTC)
        timestamp_jst = timestamp_utc.astimezone(japan_tz)
        timestamp_str = timestamp_jst.strftime("%Y-%m-%d %H:%M:%S")

        with st.expander(f"{timestamp_str} - Q: {row['question'][:50] if row['question'] else 'N/A'}..."):
